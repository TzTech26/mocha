...
// rather than under them - the proxy viewer's frame is the only one. Below xl
// there are no rails and it gets the whole window back.
...
function IsolatedUnit(props: { unit: AdUnit }) {
  ...
  const srcDocument = () => {
    ...
    return [
      ...
      `<style>html,body{margin:0;padding:0;color-scheme:${scheme};background:${ground};color:${colour};font-family:'Quicksand',sans-serif}${stackCss}</style>`,
      '</head><body>',
...
export function AdColumn(props: { placement: AdPlacement; side: AdSide; when?: boolean }) {
  ...
  return (
    <Show when={props.when !== false && units().length > 0}>
      <aside data-ad-rail={props.side} class="hidden shrink-0 flex-col gap-8 xl:flex" style={{ width: 'var(--ad-rail)' }}>
        <For each={units()}>{(unit) => <Banner unit={unit} />}</For>
      </aside>
    </Show>
  )
}
...
// pinned to the edges instead. Only the first banner in each rail, since
// without a scroll there is no way to reach a second.
